...
import time
import board
from adafruit_lsm6ds.ism330dhcx import ISM330DHCX, GyroRange
import time
import sys
import threading
...
def printNewMax(value, current_max, axis, hostname):
    if value > current_max:
        ...
        if old_max != 0:         # Evita que se imprima el primer valor cuando aún no hay maximo
            ...
            publish.single("stmki210", json.dumps(payload), hostname=hostname)
    return current_max
...
def stmki210_ISSR():
    ...
    timerObject = threading.Timer(t_max_update, stmki210_ISSR)  
